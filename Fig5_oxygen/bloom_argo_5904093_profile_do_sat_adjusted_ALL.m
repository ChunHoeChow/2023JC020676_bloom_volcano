...
% I=find(T_id==5904093 & gt(:,2)>=5 & gt(:,2)<=10 & gt(:,1)==2018); %return
I=find(T_id==5904093); %return ALL
% L=[6:2:300]; 
% return
L=[6:2:500];
...
for m=1:length(I)
    ...
    if length(Iok)>0, return; end %% double check
    pres=pres(Iok);
    do=do(Iok);
    temp=temp(Iok);
    salt=salt(Iok);    
...
%     end
    ...
    time_float=time(1)+(time(2)-1+(time(3)-1)/mon_day(time(2)))/12;
    time_all(:,p)=ones(1,length(L))*time_float;
    month_all(:,p)=ones(1,length(L))*time(2);
    gt_all(p,:)=gt(I(m),:);
...
contourf(time_all,dep_all,do_all,[100:5:300],'linecolor','none')
...
mon=month_all(I);
do=do_all(I);
plot(time,do,'.k','markersize',20)
I=find(mon>=6 & mon<=7);
hold on
plot(time(I),do(I),'.r','markersize',20)
plot([2013 2019],[213 213],'--k')
% text(6.2,150,'(b) Argo DO adjusted (micro mol/kg)','backgroundcolor','w')
xlabel('Time (year)')
ylabel('DO (micro mol/kg)')
set(gca,'xlim',[2014 2019],'fontsize',25,'linewidth',2,'xtick',[2014:2019])

time67=time(I);
do67=do(I);
save('argo_5904093_do_timeseries_above25m_2014_2019','time','do','time67','do67')
save('D:\on_going_proposal\running\science\on_going_paper\bloom_2018\files_submitted2\argo_5904093_do_timeseries_above25m_2014_2019','time','do','time67','do67')
return
figure
boxplot(do(I))
8/length(I)
...
% save('5904093_2018_oxden_gt','pden','delO2','satO2','do_all','time_all',...
%     'dep_all','salt_all','temp_all','lon_all','lat_all','satO2_clim','do_all_clim',...
%     'pden_anol','temp_anol','pden_all','temp_all','gt_all')
toc