...
% return
I=find(T_id==5902509 & gt(:,2)>=5 & gt(:,2)<=10); %return
% myfigure
for m=1:length(I)
    pres=T_pres{I(m)};
    temp=T_temp{I(m)};
...
% clabel(C,h)
[C,h]=contour(time_all,dep_all,pden_all-1000,[18:0.2:32],'linecolor','k');
clabel(C,h,[18:32])
...
% save('argo_5902509_profile_dataplot_nocloud_ADJUSTED_BGC','time_all','jd_all','time_all_year','dep_all','temp_all','pden_all','salt_all','argo_chl_all','lon_all','lat_all','n2')
