...
% return
I=find(T_id==5905219 & gt(:,2)>=6 & gt(:,2)<=10); %return
% myfigure
for m=1:length(I)
    pres=T_pres{I(m)};
    temp=T_temp{I(m)};
...
save('argo_5905219_profile_dataplot_nocloud_ADJUSTED_BGC','time_all','jd_all','time_all_year','dep_all','temp_all','pden_all','salt_all','argo_chl_all','lon_all','lat_all','n2')
