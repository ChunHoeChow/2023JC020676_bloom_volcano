...
% load('D:\on_going_proposal\running\science\on_going_paper\dragon_chla\ECMWF_winds\er5_corpenicus_wind_curl_2001_2023_div',...
%     'xi','yi','u_all','v_all','wcurl','wdiv','year','month')
%Need daily wind
pth_wind='W:\CMEMS\CMAMS_wind\';
...
% return
I=find(T_id==5905059 & gt(:,2)>=6 & gt(:,2)<=10); %return
% myfigure
for m=1:length(I)
    pres=T_pres{I(m)};
    temp=T_temp{I(m)};
...
%     argo_chl=interp2(chl_x,chl_y,chl,argo_lon,argo_lat,'nearest');
    ...
    pth_sla=['Z:\SCS initial data\web\SLA\copernicus\2021_version\',...
        num2str(time(1)),'\',num2str(time(2),'%2.2u'),'\'];
    file_sla=ls([pth_sla,'*',num2str(time(1)),num2str(time(2),'%2.2u'),...
        num2str(time(3),'%2.2u'),'*.nc']);
    in=[pth_sla,file_sla]; %return   
    lon_sla = nc_varget(in,'longitude'); 
        ilon=find(lon_sla<0); lon_sla(ilon)=lon_sla(ilon)+360;
        lon2(1:length(lon_sla)-length(ilon))=lon_sla(length(ilon)+1:end); 
        lon2(length(lon_sla)-length(ilon)+1:length(lon_sla))=lon_sla(1:length(ilon));
        lon_sla=lon2; clear lon2
    lat_sla = nc_varget(in,'latitude');
        Ix=find(lon_sla>=x1 & lon_sla<=x2);
        Iy=find(lat_sla>=y1 & lat_sla<=y2);    
    [xi,yi]=meshgrid(lon_sla(Ix),lat_sla(Iy));
    sla = nc_varget(in,'sla'); 
        chl2(:,1:length(lon_sla)-length(ilon))=sla(:,length(ilon)+1:end); 
        chl2(:,length(lon_sla)-length(ilon)+1:length(lon_sla))=sla(:,1:length(ilon)); %return
        sla=chl2; clear chl2     
    sla=sla(Iy,Ix);
%     pcolor(xi,yi,sla); shading flat
    argo_sla=griddata(xi,yi,sla,argo_lon,argo_lat);
    argo_sla_all(m)=argo_sla;
    
    file_wind=ls([pth_wind,num2str(time(1)),num2str(time(2),'%2.2u'),num2str(time(3),'%2.2u'),'*.nc']);
    for dd=1:4
        inw=[pth_wind,file_wind(dd,:)];
        lonw=nc_varget(inw,'lon');
            ilon=find(lonw<0); lonw(ilon)=lonw(ilon)+360;
            lon2(1:length(lonw)-length(ilon))=lonw(length(ilon)+1:end); 
            lon2(length(lonw)-length(ilon)+1:length(lonw))=lonw(1:length(ilon));
            lonw=lon2; clear lon2
        latw=nc_varget(inw,'lat');
        Ix=find(lonw>=x1 & lonw<=x2);
        Iy=find(latw>=y1 & latw<=y2);    
            
        spdw=nc_varget(inw,'wind_speed');
        chl2(:,1:length(lonw)-length(ilon))=spdw(:,length(ilon)+1:end); 
        chl2(:,length(lonw)-length(ilon)+1:length(lonw))=spdw(:,1:length(ilon)); %return
        spdw=chl2; clear chl2     
        spdw=spdw(Iy,Ix);
        spdw_all(:,:,dd)=spdw;
%         pcolor(xiw,yiw,spdw); shading flat
    end
    spdw_daymean=mean(spdw_all,3,'omitnan');
    clear spdw_all
    
    [xiw,yiw]=meshgrid(lonw(Ix),latw(Iy));    
    argo_spdw=griddata(xiw,yiw,spdw_daymean,argo_lon,argo_lat);
    argo_spdw_all(m)=argo_spdw;    
%     return
end
...
% save('argo_5905059_profile_dataplot_nocloud_ADJUSTED2023_BGC',...
%     'time_all','jd_all','time_all_year','dep_all',...
%     'temp_all','pden_all','salt_all','argo_chl_all',...
%     'lon_all','lat_all','n2','argo_sla_all','time_gt')
